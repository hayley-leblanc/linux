        ...
        match T::SUPER_TYPE {
            ...
            Super::Keyed => {
                from_kernel_result! {
                    ...
                    crate::pr_info!("fs data into pointer");
...
impl<'a, T: Type + ?Sized> NewSuperBlock<'a, T, NeedsInit> {
        ...
        T::Data: 'static + core::fmt::Debug,
    {
        ...
        if let Super::Keyed = T::SUPER_TYPE {
            ...
            // crate::pr_info!("setting s_fs_info to {:p}", info);
            sb.s_fs_info = info;
            // let data = unsafe { &*((*self.sb.0.get()).s_fs_info as *mut T::Data) };
            // crate::pr_info!("after setting, sbi is {:#?}", data);
        }
    ...
    /// Obtains the size and virtual address for the DAX device that the file system is
    /// mounted on
    pub fn get_dax(&mut self) -> Result<(*mut core::ffi::c_void, core::ffi::c_long)> {
        if let Super::BlockDev = T::SUPER_TYPE {
            let mut virt_addr: *mut core::ffi::c_void = ptr::null_mut();
            let mut dev_offset: u64 = 0;
            // SAFETY: `self.sb` is obtained in `get_tree_callback` so it is safe to
            // dereference it.
            // The type invariant guarantees that `self.sb` is the only pointer to a
            // newly-allocated superblock, so it is safe to mutably reference it.
            let dax_dev = unsafe {
                bindings::fs_dax_get_by_bdev((*self.sb.0.get()).s_bdev, &mut dev_offset as *mut u64)
            };
            if dax_dev.is_null() {
                Err(EINVAL)
            } else {
                // SAFETY: dax_dev is safe to access.
                // TODO: can the file system access the PM device when it is not safe to?
                let size = unsafe {
                    bindings::dax_direct_access(
                        dax_dev,
                        0,
                        (usize::MAX / crate::PAGE_SIZE).try_into()?,
                ...
                if size < 0 {
                    Err(Error::from_kernel_errno(size.try_into()?))
                } else {
                    Ok((virt_addr, size))
                }
            }
...
impl<'a, T: Type + ?Sized> NewSuperBlock<'a, T, NeedsRoot> {
    ...
    /// Obtain the fs-private info stored in the superblock.
    pub fn get_fs_info(&self) -> ScopeGuard<T::Data, fn(T::Data)> {
