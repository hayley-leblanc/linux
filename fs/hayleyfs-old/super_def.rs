...
// use kernel::bindings::{dax_device, kgid_t, kuid_t, umode_t};
...
// durable superblock.
// TODO: what other fields should this have?
#[repr(C)]
...
pub(crate) struct SbWrapper<State, Op> {
    state: PhantomData<State>,
    op: PhantomData<Op>,
    sb: &'static HayleyfsSuperBlock,
}

impl SbWrapper<Clean, Init> {
    pub(crate) fn init(sbi: &SbInfo) -> SbWrapper<Clean, Init> {
        let mut sb = unsafe { &*(sbi.danger_get_pm_addr() as *mut HayleyfsSuperBlock) };
        sb.magic = HAYLEYFS_MAGIC;
        sb.size = sbi.pm_size;
        clwb(sb, size_of::<HayleyfsSuperBlock>(), true);
        Self {
            state: PhantomData,
            op: PhantomData,
            sb,
...
pub(crate) struct SbInfo {
    ...
    pub(crate) virt_addr: *mut core::ffi::c_void, // raw pointer virtual address of beginning of FS instance
